...
{
	dma_free_wc(fbi->dev, fbi->map_size_f1, fbi->map_cpu_f1,  fbi->map_dma_f1);

#if defined(CONFIG_FB_S3C_EXT_DOUBLE_BUFFERING)
	dma_free_wc(fbi->dev, fbi->map_size_f2, fbi->map_cpu_f2,  fbi->map_dma_f2);
#endif
...
static int s3cfb_sysfs_store_lcd_power(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (strncmp(buf, "on", 2) == 0 || strncmp(buf, "1", 1) == 0)
		s3cfb_set_lcd_power(1);
	else if (strncmp(buf, "off", 3) == 0 || strncmp(buf, "0", 1) == 0)
		s3cfb_set_lcd_power(0);
	else
		return -EINVAL;
